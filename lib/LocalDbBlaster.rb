...
class LocalDbBlaster
    ...
    def submitTblastxQuery(query_filename,out_filename)
        %x(tblastx -db /home0/jburkhar/biting_microarray/transcriptome/ftp.ncbi.nlm.nih.gov/refseq/release/invertebrate/invertebrate_rna.fna -evalue 1.0e-5 -query #{query_filename} -num_threads 32 -out #{out_filename} -outfmt "10 qseqid sacc evalue" -max_target_seqs 2 -culling_limit 1)
    end
